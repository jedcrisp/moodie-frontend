...
export default function AdminDashboard({ user }) {
  ...
  // Navigate to mood selector (relative routing)
  ...
  const saveNote = async (id) => {
    ...
    try {
      ...
      setLoading(true);
      const studentRef2 = collection(db, 'schools', user.school, 'students');
      const studentSnap2 = await getDocs(studentRef2);
      const studentData2 = await Promise.all(
        studentSnap2.docs.map(async (docSnap) => {
          const student = docSnap.data();
          const moodsRef = collection(
            db,
            'schools',
      ...
      const sorted2 = studentData2.sort((a, b) => (a.averageMood ?? 99) - (b.averageMood ?? 99));
      setStudents(sorted2);
      setLoading(false);
    } catch (err) {
